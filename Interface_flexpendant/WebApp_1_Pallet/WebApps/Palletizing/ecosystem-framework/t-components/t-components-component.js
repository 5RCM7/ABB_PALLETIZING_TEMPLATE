...
export class Component_A extends Base_A {
  ...
  async init() {
    try {
      ...
      if (typeof this._deinstallFunction === 'function')
        this._deinstallFunction();
      /**
       * Parent HTML element where the component is attached to.
       */
      this.parent;
  ...
  async initChildrenComponents() {
    ...
    if (!this.child) {
      ...
      for (const key in newChildren) {
        ...
        if (Component_A._isTComponent(oldChild)) {
          const shouldUpdate =
            oldChild._fUpdate ||
            !Component_A._equalProps(oldChild._props, newChild._props);
    ...
    const status = this._props.options.async
      ? initChildren()
      : await Promise.all(initChildren());
  ...
  destroy() {
    ...
    if (
      this._deinstallFunction &&
      typeof this._deinstallFunction === 'function'
    )
      this._deinstallFunction();
  ...
  _markupWithLabel() {
    return /*html*/ `
        ${Component_A.mIf(
          this._props.label &&
            (this._props.labelPos === 'top' || this._props.labelPos === 'left'),
          /*html*/ `<p id="${this.compId}__label"></p>`
        )}
        ${this.markup(this)}
        ${Component_A.mIf(
          this._props.label &&
            (this._props.labelPos === 'bottom' ||
              this._props.labelPos === 'right'),
          /*html*/ `<p id="${this.compId}__label"></p>`
        )}
    `;
  }
  ...
  attachToElement(element) {
    ...
    if (!this.parent) {
      ...
      this.parent.contains(this.container) === false &&
        this.parent.appendChild(this.container);
    } else {
      // remove from old parent to attach to new one
      this.parent.contains(this.container) &&
  ...
  all(selector) {
    ...
    var nlTemplate =
      this.template && this.template.content.querySelectorAll(selector);
  ...
  static _equalProps(newProps, prevProps) {
    ...
    const stringify = (obj) => {
      return JSON.stringify(obj, (key, value) => {
        if (Component_A._isTComponent(value)) {
          return value.compId;
        }

...
export class Hamburger_A extends Menu_A {
  ...
  onRender() {
    ...
    this.find('.fp-components-hamburgermenu-a-container').style.alignItems =
      'normal';
    this.find('.fp-components-hamburgermenu-a-container').classList.add(
      'hamburger__container'
    );
    this.find(
      '.fp-components-hamburgermenu-a-container__content'
    ).classList.add('hamburger__container', 'flex-col', 'justify-stretch');
  ...
  set activeView(name) {
    this.all('.fp-components-hamburgermenu-a-menu__button--active').forEach(
      (el) => {
        el.classList.remove(

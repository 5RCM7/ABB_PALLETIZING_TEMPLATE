...
export class Base_A extends Eventing_A {
  ...
  initPropsDep(props) {
    if (typeof props === 'string') {
      ...
      throw new Error(
        'The new value should be a string or an array of strings.'
      );
    }
    ...
    let props = Object.keys(prevProps).reduce((acc, key) => {
      if (newProps.hasOwnProperty(key)) {
        if (
          ...
          const nestedProps = this._updateProps(
            newProps[key],
            prevProps[key],
            restError
          );
  ...
  async _componentDidUpdate() {
    ...
    const isDepsDiff =
      this._initPropsDependencies &&
      checkDepsDiff(this._initPropsDependencies, this._props, this._prevProps);
